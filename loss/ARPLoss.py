...
class ARPLoss(nn.CrossEntropyLoss):
    ...
    def forward(self, x, y, labels=None):#todo 特征(21X64),(21X8)，真实标签(21X1)
        dist_dot_p = self.Dist(x, center=self.points, metric='dot')#todo [21,8]
        dist_l2_p = self.Dist(x, center=self.points,metric='l2')#todo [21,8]
        logits = dist_l2_p - dist_dot_p#todo 计算l2和dot之间的距离 [21,8]
        ...
        loss_r = self.margin_loss(self.radius, _dis_known, target)#todo 计算边缘损失
